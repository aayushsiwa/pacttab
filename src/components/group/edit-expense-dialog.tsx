...
function EditExpenseForm({
  ...
  expense.splits.forEach((s) => {
    initialExact[s.userId] = Number(s.owedAmount).toFixed(2);
  });
  const [exactValues, setExactValues] = useState<Record<string, string>>(initialExact);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await updateExpenseAction(null, formData);
      if (res.success) {
        toast.success(`Updated expense: ${description}`);
        onClose();
        router.refresh();
      } else {
